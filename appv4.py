...
def avaliar():
    ...
    if request.method == "POST":
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO avaliacoes (usuario_id, produto, nota, comentario)
            VALUES (%s, %s, %s, %s)
        """, (session["usuario_id"], produto, nota, comentario))
        conn.commit()
        conn.close()

        return "<h2>Avaliação registrada com sucesso!</h2><a href='/avaliar'>Fazer outra</a>"
...
def avaliacoes():
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            SELECT a.produto, a.nota, a.comentario, u.nome
            FROM avaliacoes a
            JOIN usuarios u ON a.usuario_id = u.id
            WHERE a.produto LIKE %s
        """, ('%' + termo + '%',))
    else:
        cur.execute("""
            SELECT a.produto, a.nota, a.comentario, u.nome
    ...
    dados = cur.fetchall()
    conn.close()
    return render_template("avaliacoes.html", avaliacoes=dados, termo=termo)
